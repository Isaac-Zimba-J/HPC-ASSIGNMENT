...
# # Define hypercube dimension (4D = 16 nodes)
# D = 4
...
# ax.set_title(f"{D}-Dimensional Hypercube Simulation")

# # Draw static elements (nodes, edges)
# nx.draw(G, pos, with_labels=True, node_color="lightgray", edge_color="gray", node_size=800, font_size=10)

# # Nodes that will send messages
...
#         nx.draw_networkx_nodes(G, pos, nodelist=[sender], node_color="red", node_size=800)
#         nx.draw_networkx_nodes(G, pos, nodelist=[receiver], node_color="green", node_size=800)
#         nx.draw_networkx_edges(G, pos, edgelist=[(sender, receiver)], edge_color="blue", width=2)

# # Run animation
# ani = animation.FuncAnimation(fig, update, frames=20, interval=1000, repeat=True)
# plt.show()
